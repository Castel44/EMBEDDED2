...
# Building model
print('ELM')
elm = hpelm.ELM(X_train.shape[1],out_class, classification ="c", batch=batch_size, precision=prec)
...
print('\nELM-GPU double')
elm = hpelm.ELM(X_train.shape[1],out_class, classification ="c", batch=batch_size, accelerator="GPU", precision='double')
elm.add_neurons(neuron_number,'sigm')
print(str(elm))
# Training model
t = time.time()
