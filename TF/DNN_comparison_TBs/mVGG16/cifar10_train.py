...
model.fit_generator(datagen_train.flow(x_train, y_train,
                                  ...
                                  epochs=n_epochs,
                                  validation_data=datagen_test.flow(x_test, y_test, batch_size=batch_size), callbacks=[reduce_lr, tensorboard])


model.save_weights(log_dir +'/cifar10vgg2.h5')
...
with open(log_dir + "/hyperpars.txt", "w") as text_file:
    ...
    print("zca_epsilon: {}".format(zca_epsilon), file=text_file)
    print("lr_decay: {}".format(lr_decay), file=text_file)
