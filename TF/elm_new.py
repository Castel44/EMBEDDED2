    ...
    def train(self, tf_iterator, n_batches=None):
        ...
        while True:
            try:
                ...
                # Run the training op
                self.sess.run(self.HH_HT_op, feed_dict={self.x: x_batch, self.y: y_batch})
...
def main():
    ...
    import time
    t = time.time()
    elm1 = elm(input_size=input_size, output_size=output_size, l2norm=None)
    elm1.add_layer(4096, activation=tf.sigmoid)
    elm1.compile()
    ...
    def gen():
        ...
        for x, y in datagen.flow(x_train, y_train, batch_size=batch_size):
            x = x.reshape(batch_size, x_train.shape[1] * x_train.shape[1])
            if n_it % 100 == 0:
